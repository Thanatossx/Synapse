...
export const DATA_PATH = path.join(__dirname, "data", "store.json");
...
export async function readStore() {
  try {
    ...
    // Backfill new fields without breaking existing stores.
    let changed = false;
    ...
    for (const ev of parsed.events) {
      ...
      if (!ev.applicationForm || typeof ev.applicationForm !== "object") {
        ev.applicationForm = {
          version: 1,
          questions: [
            {
        ...
        if (typeof ev.applicationForm.version !== "number") {
          ev.applicationForm.version = 1;
          changed = true;
        }
        if (!Array.isArray(ev.applicationForm.questions)) {
          ev.applicationForm.questions = [];
          changed = true;
        }
      }
    }
